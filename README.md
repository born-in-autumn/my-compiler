...
NOTE：
✅ 补完成后端：Binary 四则 + Neg + Store-Temp + Print-Const + 修 offset bug。做完纯算术语言真正稳了。
比较 == != < <= > >= + 布尔 true/false。表达式级、无跳转；&& || 先跳过（短路本质是分支）。
if/else——最大一道坎：IR 现在没有 Label/Jump/条件跳转，得先加它，再落到汇编 cmp/b.cond。
块 + 作用域 + scope 栈。语义分析从这里冒头。
while/for。复用第 3 步的跳转机器，for 脱糖成 while，增量很小。
函数/return/调用。参数、通用化 prologue/epilogue，大概率正式启用 x29。
字符串字面量 + 打印。
类型检查/未定义变量报错补全。
优化（垫底）：寄存器分配、常量传播、死代码、真 SSA，全绿后再碰。